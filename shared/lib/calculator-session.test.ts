...
import { createCalculatorSessionState, isCalculatorSessionDirty, reduceCalculatorSession } from './calculator-session';

describe('calculator session state', () => {
  it('keeps a committed result while a user edits a draft', () => {
    const initial = createCalculatorSessionState<{ amount: number }, { total: number }>({ amount: 100 });
    const committed = reduceCalculatorSession(initial, { type: 'succeed', committedInputs: { amount: 100 }, committedResult: { total: 110 } });
    const edited = reduceCalculatorSession(committed, { type: 'set-draft', draftInputs: { amount: 200 } });
  ...
  it('commits only an explicit successful calculation', () => {
    ...
    const completed = reduceCalculatorSession(running, { type: 'succeed', committedInputs: { amount: 100 }, committedResult: 105 });
  ...
  it('does not discard a committed result after failure or cancellation', () => {
    const initial = reduceCalculatorSession(createCalculatorSessionState<{ amount: number }, number>({ amount: 100 }), { type: 'succeed', committedInputs: { amount: 100 }, committedResult: 105 });
  ...
  it('restores a draft and committed snapshot independently', () => {
    ...
    const restored = reduceCalculatorSession(initial, { type: 'restore', draftInputs: { amount: 120 }, committedInputs: { amount: 100 }, committedResult: 111 });
