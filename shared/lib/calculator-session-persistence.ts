...
export interface RestoredCalculatorSession<TInputs, TResult> extends PersistedCalculatorSession<
  TInputs,
  TResult
> {
...
export function restoreCalculatorSession<TInputs, TResult>(
  ...
  const validResult =
    persisted.committedResult !== null &&
    isCommittedResultValid(persisted.committedResult) &&
    (!expectedModelVersion || persisted.modelVersion === expectedModelVersion);
  return {
...
export function applyUntouchedMacroDefaults<
  T extends { expectedInflation?: number; expectedNbpRate?: number },
>(
  inputs: T,
...
export function createPersistedCalculatorSession<TInputs, TResult>(
  ...
  return { modelVersion, draftInputs, committedInputs, committedResult };
}
