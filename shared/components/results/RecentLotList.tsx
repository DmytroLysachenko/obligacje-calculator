  ...
  return (
    <section className={cn('space-y-5 border-y border-border py-6', className)}>
      <div className={compact ? 'space-y-1.5' : 'space-y-2'}>
        ...
        <p className={compact ? 'text-sm leading-6 text-muted-foreground' : 'ui-body text-muted-foreground'}>
          {description}
        </p>
        {note ? (
          <p className={cn(
      ...
      <div className="divide-y divide-border">
        {items.map((item) => (
          <article key={item.key} className={compact ? 'py-3 first:pt-0 last:pb-0' : 'py-4 first:pt-0 last:pb-0'}>
