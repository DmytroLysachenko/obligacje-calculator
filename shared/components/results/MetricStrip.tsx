  ...
  return (
    <section className={cn('border-y border-border', className)}>
      <div className={cn('grid divide-y divide-border md:divide-y-0', columns)}>
        {items.map((item) => (
          <div key={item.label} className="min-w-0 space-y-2 py-4 md:border-l md:border-border md:px-4 md:first:border-l-0 md:first:pl-0">
            <p className="ui-meta font-semibold">{item.label}</p>
            <p className={cn('financial-number ui-large-metric min-w-0 break-words text-foreground', item.tone)}>
              {item.value}
            </p>
            {item.description ? (
