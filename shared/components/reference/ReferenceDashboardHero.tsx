  ...
  return (
    <section className="border-y border-border py-5 md:py-6">
      <div className="space-y-4">
        ...
        {decisionTrace ? (
          <div className="border-l-2 border-success/70 bg-success/5 px-4 py-3">{decisionTrace}</div>
        ) : null}
        {metrics?.length ? (
          <div className="grid gap-x-8 gap-y-5 border-t border-border pt-4 sm:grid-cols-2">
            {metrics.map((metric) => (
              <div
                key={metric.label}
