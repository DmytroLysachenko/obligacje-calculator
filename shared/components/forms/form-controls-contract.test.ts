...
describe('shared form control contracts', () => {
  ...
  it('migrates repeated calculator controls to the shared form primitives', () => {
    ...
    expectContains(singleTiming, "import { FormSelect } from '@/shared/components/forms/FormSelect';");
    expectContains(singleTiming, '<FormSelect');
    expectContains(regularContribution, "import { FormSelect } from '@/shared/components/forms/FormSelect';");
    expectContains(regularContribution, "import { MoneyInput } from '@/shared/components/forms/MoneyInput';");
    expectContains(regularContribution, '<FormSelect');
    expectContains(regularContribution, '<MoneyInput');

    expectNotContains(singleTiming, "import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';");
    expectNotContains(regularContribution, "import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';");
    expectNotContains(regularTiming, "import { CommittedSliderInput } from '@/shared/components/CommittedSliderInput';");
  });
});
