...
| SEC-13     | In progress                    | `6a524b6 fix(security): escape inline JSON-LD` preserves nonce CSP and tests safe serialization; COOP/CORP now isolate browsing contexts and same-origin resources without an incompatible COEP policy. CSP reporting and style-attribute reduction remain.         |
...
| ARC-09     | In progress                    | `feat(ui): add locale financial formatters` centralizes date, currency, percentage, and compact-number display decisions; the remaining Phosphor primitives now use the established Lucide system.                                                                  |
