...
| TEST-05    | In progress                    | V8 coverage targets high-risk calculation, server, and shared-library source with a checked aggregate threshold ratchet; full coverage verification must pass before completion.                                                                                                                                                                                                                                                           |
...
| COR-04     | In progress                    | Exact model-version envelopes are mandatory and persisted results restore only on an exact match. Versioned golden fixtures and complete historical-result policy remain.                                                                                                                                                                                                                                                                  |
...
| REL-04     | In progress                    | Readiness validates runtime configuration, dependency connectivity, required operational tables, and reviewed migration-journal count with strict database timeouts. Deployed readiness receipt remains external evidence.                                                                                                                                                                                                                |
...
| ARC-08     | In progress                    | Verified and removed the unreachable legacy GUS sync provider, public opportunistic-sync path, unused `MoneyInput`, chart-sync context, unused `ToolCard`, and several unconsumed helpers. The unused scan now reports no production files or exports; remaining type/config findings require consumer verification.                                                                                                                     |
...
| DOC-05     | In progress                    | Current UI direction, rules, design-system, and accessibility contracts live under numbered `docs/ui/`; archive-link and stale-claim review remains.                                                                                                                                                                                                                                                                                     |
