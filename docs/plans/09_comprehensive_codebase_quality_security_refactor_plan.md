...
| SEC-06     | In progress | `fix(portfolio): bound import payloads` sets the guest ownership cookie `Secure` in production. Origin/Sec-Fetch-Site validation and a documented CSRF boundary remain.                                                                                                                           |
| COR-01     | In progress | Uncommitted working-tree changes load freshness/definitions concurrently and add freshness metadata to the calculation cache key; tests, TTL/namespace behavior, and a commit remain.                                                                                                             |
| REL-01     | In progress | Uncommitted working-tree changes replace `null as T` cancellation with `CalculationCancelled` and preserve the prior session result; tests and a commit remain.                                                                                                                                   |
| ARC-01     | In progress | `fix(calculation): version cache and cancel safely` will consolidate cancellation-safe request ownership; calculator-specific adapters remain outside the shared workflow.                                                                                                                        |
| ARC-02     | In progress | `fix(calculation): version cache and cancel safely` will make freshness context and cache identity an explicit calculation-service contract.                                                                                                                                                      |
| PERF-05    | In progress | `fix(calculation): version cache and cancel safely` loads independent definitions and freshness concurrently before cache lookup.                                                                                                                                                                 |
| SCALE-01   | In progress | `fix(calculation): version cache and cancel safely` treats the local cache as an optimization keyed by declared authority, never correctness state.                                                                                                                                               |
...
| TEST-01    | In progress | `test(quality): isolate Vitest from browser suites` adds explicit runner ownership; the complete full-suite remediation remains in CI.                                                                                                                                                           |
...
| SEC-13     | In progress | `fix(security): escape inline JSON-LD` preserves nonce CSP while preventing JSON-LD script termination; reporting and COOP/CORP production evidence remain.                                                                                                                                 |
...
| UI-04      | In progress | `feat(ui): add locale financial formatters` provides one tested locale-aware formatter surface; route-by-route adoption remains.                                                                                                                                                                |
