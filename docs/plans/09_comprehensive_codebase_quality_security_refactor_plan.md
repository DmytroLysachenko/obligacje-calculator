...
| TEST-04    | In progress | `docs(operations): restore current runtime and data-processing truth` adds a real PostgreSQL migration/constraint/rollback harness gated on an isolated `TEST_DATABASE_URL`; CI provisioning and a successful database-run artifact remain.                                                     |
