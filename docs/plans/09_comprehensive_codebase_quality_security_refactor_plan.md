...
| SEC-13     | In progress                    | `6a524b6 fix(security): escape inline JSON-LD` preserves nonce CSP and tests safe serialization; COOP/CORP now isolate browsing contexts and same-origin resources without an incompatible COEP policy. `b4f80fd3 fix(security): add private CSP reporting` adds bounded, schema-validated, sampled reports redacted before logging and a stable unsupported-media contract. Runtime `style-src-attr 'unsafe-inline'` reduction remains. |
...
| REL-03     | In progress                    | `feat(server): add correlated API responses` adds stable request IDs to wrapped successes, rate limits, and problems; admin sync now also uses the shared error/rate-limit boundary, while operational/auth adapters remain intentionally bespoke.                                                                                                                                                                                       |
...
| ARC-06     | In progress                    | Bond-definition loading now lives behind a route-scoped boundary for calculator, comparison, education, economic-data, notebook, and shared-scenario flows. The root no longer mounts global tooltip or public-sync client infrastructure; static-safe CSP rendering remains.                                                                                                                                                            |
