...
| COR-01     | In progress | `70e420f fix(calculation): version cache and cancel safely` includes freshness in the cache identity and tests concurrent authority loading; TTL, tax revision, and sync namespace invalidation remain.                                                                                           |
...
| DATA-03    | In progress | `feat(portfolio): make imports transactional and owner-scoped` folds delete predicates into repository queries; update predicates and migrated PostgreSQL coverage remain.                                                                                                                        |
