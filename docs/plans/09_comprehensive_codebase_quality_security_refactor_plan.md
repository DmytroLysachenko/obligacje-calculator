...
| ARC-07     | In progress                    | `08881e16 refactor(calculator): isolate result actions` moves single-calculator result effects behind a dedicated action module. `c8c0bc43 refactor(comparison): split result views` leaves comparison URL/state decisions in its container while draft workspace, committed-result presentation, and collapsed-plan receipt own views; focused receipt interaction coverage proves edit/close handoffs. Further hook ownership remains. |
...
| TEST-03    | In progress                    | `08881e16 refactor(calculator): isolate result actions` and `c8c0bc43 refactor(comparison): split result views` add component-adjacent action coverage and receipt edit/close interaction coverage. Input-validation, cancellation, focus-return, and portfolio/admin flows still need owned interaction suites.                                                                                                                         |
...
| TEST-06    | In progress                    | Expanded Chromium accessibility coverage across trusted public routes, an English localized axe scan, keyboard skip-link use, and 200% zoom checks. The comparison axe baseline and Polish localized scan still need remediation before they can be gates.                                                                                                                                                                               |
...
| PERF-02    | In progress                    | Bond definitions are route-scoped; global tooltip and public opportunistic-sync client infrastructure are removed. The remaining root client boundaries require bundle evidence.                                                                                                                                                                                                                                                         |
...
| UI-05      | In progress                    | Financial decision explanations for comparison results, tax configuration, and projected-rate assumptions now use 16px mobile copy with comfortable line height; the wider 320px/400% and Polish-copy audit remains.                                                                                                                                                                                                                     |
...
| UI-09      | In progress                    | Shared loading, error, live-region, and focus semantics remain. `10d31351 fix(a11y): associate purchase-date errors` gives single-calculator purchase date an explicit label, invalid state, and associated inline error; submission focus handling and wider workflow coverage remain.                                                                                                                                                  |
