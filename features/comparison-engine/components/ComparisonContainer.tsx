    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <div className="space-y-6">
          <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 px-4 py-1 font-bold uppercase tracking-widest text-[10px]">Scenario A</Badge>
          <BondInputsForm inputs={inputsA} onUpdate={updateInputA} onBondTypeChange={setBondTypeA} />
        </div>
        <div className="space-y-6">
          <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 px-4 py-1 font-bold uppercase tracking-widest text-[10px]">Scenario B</Badge>
      ...
      {!resultsA && !isCalculating && (
        <div className="h-[400px] flex flex-col items-center justify-center border-2 border-dashed rounded-3xl opacity-50 space-y-4">
          ...
          <p className="font-medium text-muted-foreground">Adjust inputs and click &apos;Compare Scenarios&apos; to see results</p>
        </div>
      )}
      ...
      {isCalculating && !resultsA && (
        <div className="h-[400px] flex flex-col items-center justify-center space-y-4">
          ...
          <p className="font-medium text-primary">Analyzing both scenarios...</p>
        </div>
      )}
        ...
        <div className={cn("space-y-8 transition-opacity duration-300", isCalculating && "opacity-50 pointer-events-none")}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card className="border-none shadow-lg bg-gradient-to-br from-blue-50 to-white rounded-3xl overflow-hidden">
              <CardHeader className="border-b border-blue-100/50 bg-blue-100/20"><CardTitle className="text-lg font-black text-blue-900 uppercase tracking-widest">Scenario A Summary</CardTitle></CardHeader>
              <CardContent className="space-y-4 pt-6">
                <div className="flex justify-between items-end border-b border-blue-100 pb-4">
                  <span className="text-xs font-bold text-blue-800/60 uppercase">Net Payout:</span>
                  <span className="text-3xl font-black text-blue-700">{formatCurrency(resultsA.netPayoutValue)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-blue-800/60 uppercase">Total Profit:</span>
            ...
            <Card className="border-none shadow-lg bg-gradient-to-br from-emerald-50 to-white rounded-3xl overflow-hidden">
              <CardHeader className="border-b border-emerald-100/50 bg-emerald-100/20"><CardTitle className="text-lg font-black text-emerald-900 uppercase tracking-widest">Scenario B Summary</CardTitle></CardHeader>
              <CardContent className="space-y-4 pt-6">
                <div className="flex justify-between items-end border-b border-emerald-100 pb-4">
                  <span className="text-xs font-bold text-emerald-800/60 uppercase">Net Payout:</span>
                  <span className="text-3xl font-black text-emerald-700">{formatCurrency(resultsB.netPayoutValue)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-emerald-800/60 uppercase">Total Profit:</span>
