...
const assets = [
  asset('edo', 'EDO', '#0f766e'),
  asset('sp500', 'S&P 500', '#2563eb'),
];
...
describe('multi asset chart model', () => {
  it('creates growth summaries from the last chart point', () => {
    expect(createMultiAssetGrowthSummary({
      chartData,
      assets,
      formatCurrency: (value) => `PLN ${value}`,
      labels: {
        empty: () => 'empty',
        populated: (values) => `${values.count}:${values.leader}:${values.leaderValue}:${values.trailing}:${values.trailingValue}`,
      },
  ...
  it('creates drawdown summaries from the deepest last-point drawdown', () => {
    expect(createMultiAssetDrawdownSummary({
      chartData,
      assets,
      labels: {
        empty: () => 'empty',
