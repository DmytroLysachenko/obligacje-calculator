...
export const BondComparisonContainer = () => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setCustomInflation(
        Array.from(
          { length: nextLength },
          (_, index) => customInflation[index] ?? expectedInflation,
        ),
      );
    }, 0);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setCustomNbpRate(
        Array.from({ length: nextLength }, (_, index) => customNbpRate[index] ?? expectedNbpRate),
      );
    }, 0);
    ...
    if (key === 'customNbpRate') {
      ...
      setCustomNbpRate(
        nextPath
          ? Array.from(
              { length: Math.max(1, Math.round(duration)) },
              (_, index) => nextPath[index] ?? expectedNbpRate,
            )
          : undefined,
      );
    }
  ...
  const chartData = useMemo(
    () => buildComparisonChartData(results, showRealValue),
