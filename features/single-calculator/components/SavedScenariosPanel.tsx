...
export function SavedScenariosPanel({ scenarios, onSaveCurrent, onLoad, onDuplicate, onDelete, }: SavedScenariosPanelProps) {
    ...
    return (<section className="space-y-4">
      ...
      <div className="ui-divider-group">
        {scenarios.length === 0 ? (<div className="py-4 text-sm text-muted-foreground">
            {t('single_calculator.saved_scenarios.empty')}
          </div>) : (scenarios.map((scenario) => (<div key={scenario.id} className="py-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="ui-card-title">{scenario.name}</h3>
                    {scenario.tags.map((tag) => (<Badge key={tag} variant="outline" className="text-[10px] font-semibold uppercase tracking-[0.08em]">
                        {tag}
                      </Badge>))}
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 lg:w-[260px]">
                  <Button variant="outline" size="sm" className="gap-2 text-xs font-semibold" onClick={() => onLoad(scenario)}>
                    <FolderOpen className="h-3.5 w-3.5"/>
                    {t('single_calculator.saved_scenarios.open')}
                  </Button>
                  <Button variant="outline" size="sm" className="gap-2 text-xs font-semibold" onClick={() => onDuplicate(scenario)}>
                    <Copy className="h-3.5 w-3.5"/>
                    {t('single_calculator.saved_scenarios.copy')}
                  </Button>
                  <Button variant="outline" size="sm" className="gap-2 text-xs font-semibold text-destructive" onClick={() => onDelete(scenario)}>
