...
export const InflationChart = ({ period = 'ALL', }: {
    ...
    return (<ReferenceChartFrame sourceLabel={t('economic.compact_source_header')} metaItems={getReferenceMetaItems(response, language)} actions={<div className="flex gap-2">
          ...
          <Button type="button" size="sm" variant={scaleMode === 'full' ? 'default' : 'outline'} onClick={() => setScaleMode('full')} className="rounded-md">
            ...
            ? t('economic.fallback_notices.inflation_warning') : t('economic.fallback_notices.inflation_live')} fallbackTone={response?.usedFallback ? 'warning' : 'good'} fallbackStatusLabel={t('economic.reference_state.fallback')} syncedStatusLabel={t('economic.reference_state.synced')}>
