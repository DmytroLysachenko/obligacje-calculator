...
export function generateAdvisorTips(
  ...
  if (
    (bondType === BondType.TOS || bondType === BondType.OTS) &&
    expectedInflation > expectedNbpRate
  ) {
    tips.push({
      id: 'inflation_risk',
      type: 'warning',
      ...
      message:
        'Current inflation expectations are higher than the NBP rate. Short-term fixed-rate bonds might lose real purchasing power. Consider inflation-indexed bonds like COI or EDO for better long-term protection.',
    });
  }
  ...
  if (result.finalRealValue && result.finalRealValue > result.finalNominalValue) {
    tips.push({
      ...
      message:
        'This investment beats the expected inflation curve, increasing your real purchasing power over the selected horizon.',
    });
  }
