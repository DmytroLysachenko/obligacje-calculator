...
function PortfolioMiniStat({ label, value, description, }: {
    label: string;
    value: string;
    description: string;
}) {
    return (<div className="rounded-[1.5rem] border border-white/80 bg-white/78 px-4 py-4 shadow-[0_16px_32px_-28px_rgba(15,23,42,0.35)] backdrop-blur">
...
export const PortfolioDetails: React.FC<PortfolioDetailsProps> = ({ portfolio, onBack, onDelete, onPortfolioUpdate, }) => {
    ...
    const handleExport = async (formatName: 'portfolio' | 'package') => {
        try {
            const response = await fetch(`/api/portfolio/export?portfolioId=${portfolio.id}&format=${formatName}`);
            const data = await response.json();
            if (!response.ok) {
        ...
        <CardContent className="space-y-6 p-6 lg:p-7">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:w-[360px]">
              <PortfolioMiniStat label={t('notebook.stored_lots_label')} value={String(lots.length)} description={t('notebook.stored_lots_card_desc')}/>
              <PortfolioMiniStat label={t('notebook.next_maturity_label')} value={nextMaturity ? format(nextMaturity.maturityDate, 'dd.MM.yyyy') : '-'} description={t('notebook.next_maturity_card_desc')}/>
            </div>
          </div>

          <div className="grid gap-4 md:grid-cols-3">
            <PortfolioMiniStat label={t('notebook.total_invested')} value={formatCurrency(totalValue)} description={t("generated.features.notebook.components.portfolio_details.item_2")}/>
            <PortfolioMiniStat label={t('notebook.next_maturity')} value={nextMaturity ? nextMaturity.bondType : '-'} description={nextMaturity
            ? `${format(nextMaturity.maturityDate, 'dd.MM.yyyy')} ${t("generated.features.notebook.components.portfolio_details.item_3")}`
            : t("generated.features.notebook.components.portfolio_details.item_4")}/>
            <PortfolioMiniStat label={t("generated.features.notebook.components.portfolio_details.item_5")} value={isPublic ? t('notebook.public') : t('notebook.private')} description={isPublic
            ? t("generated.features.notebook.components.portfolio_details.item_6") : t("generated.features.notebook.components.portfolio_details.item_7")}/>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col gap-4 rounded-[1.8rem] border border-slate-200 bg-white/84 p-5 shadow-[0_18px_40px_-36px_rgba(15,23,42,0.35)] backdrop-blur md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {onDelete ? (<Button variant="outline" className="gap-2 border-destructive/20 bg-background text-destructive hover:bg-destructive/5 hover:text-destructive" onClick={async () => {
              ...
              {t("generated.features.notebook.components.portfolio_details.item_8")}
            </Button>) : null}
          ...
          <Button variant={isPublic ? 'default' : 'outline'} className={cn('gap-2', !isPublic && 'bg-background')} onClick={handleToggleShare} disabled={isSharing}>
        ...
        <TabsContent value="lots" className="space-y-6">
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.4fr_0.8fr]">
            <Card className="rounded-2xl border shadow-none">
              ...
              <CardContent>
                {isLoading ? (<div className="flex min-h-48 items-center justify-center gap-3 text-sm text-muted-foreground">
                    <Loader2 className="h-5 w-5 animate-spin"/>
                    {t('notebook.updating')}
                  </div>) : lots.length === 0 ? (<div className="rounded-2xl border border-dashed px-6 py-12 text-center">
                    <p className="text-sm text-muted-foreground">{t('notebook.no_lots')}</p>
                  </div>) : (<div className="rounded-2xl border border-slate-200 bg-white">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="hover:bg-transparent">
                          <TableHead className="h-12 text-sm font-semibold text-slate-600">{t("generated.features.notebook.components.portfolio_details.item_9")}</TableHead>
                          <TableHead className="h-12 text-sm font-semibold text-slate-600">{t("generated.features.notebook.components.portfolio_details.item_10")}</TableHead>
                          <TableHead className="h-12 text-right text-sm font-semibold text-slate-600">{t("generated.features.notebook.components.portfolio_details.item_11")}</TableHead>
                          <TableHead className="h-12 text-sm font-semibold text-slate-600">{t("generated.features.notebook.components.portfolio_details.item_12")}</TableHead>
                          <TableHead className="h-12 text-right text-sm font-semibold text-slate-600">{t("generated.features.notebook.components.portfolio_details.item_13")}</TableHead>
                          <TableHead className="h-12 text-right text-sm font-semibold text-slate-600">{t("generated.features.notebook.components.portfolio_details.item_14")}</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {lots.map((lot) => (<TableRow key={lot.id} className="odd:bg-slate-50/30 hover:bg-slate-50/80">
                            <TableCell className="py-4 font-medium">{lot.bondType}</TableCell>
                            <TableCell className="py-4 text-slate-600">
                              {formatBondDuration(definitions[lot.bondType as BondType]?.duration ?? 1, language)}
                            </TableCell>
            ...
            <div className="space-y-6">
              <Card className="rounded-2xl border shadow-none">
                ...
                <CardContent className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {[30, 90, 180].map((days) => (<Button key={days} variant={maturityWindowDays === days ? 'default' : 'outline'} className="h-9" onClick={() => setMaturityWindowDays(days as MaturityWindow)}>
                        {days}d
                      </Button>))}
                  </div>
                  ...
                  {filteredMaturities.length === 0 ? (<p className="text-sm text-muted-foreground">{t('notebook.no_maturities_in_window')}</p>) : (<div className="space-y-3">
                      {filteredMaturities.slice(0, 6).map((item) => (<div key={item.id} className="rounded-2xl border p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-xs font-semibold text-muted-foreground">
                                {formatBondDuration(definitions[item.bondType as BondType]?.duration ?? 1, language)}
                              </p>
                            </div>
        ...
        <TabsContent value="analytics">
            ...
            <CardContent className="space-y-4">
              ...
              {isSimulating ? (<div className="flex min-h-[320px] items-center justify-center gap-3 text-sm text-muted-foreground">
                  <Loader2 className="h-5 w-5 animate-spin"/>
                  {t("generated.features.notebook.components.portfolio_details.item_15")}
                ...
                : simulation.aggregatedTimeline} margin={{ top: 12, right: 12, left: 0, bottom: 0 }}>
                      <defs>
                      ...
                      <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(value) => format(new Date(value), 'yyyy')} minTickGap={48}/>
                ...
                t("generated.features.notebook.components.portfolio_details.item_16"),
            ]}/>
                      <Area type="monotone" dataKey="totalNetValue" stroke="#2563eb" strokeWidth={3} fill="url(#portfolioNet)" isAnimationActive={false}/>
                    </AreaChart>
                  </ResponsiveContainer>
                </ChartContainer>) : (<div className="flex min-h-[320px] items-center justify-center text-sm text-muted-foreground">
