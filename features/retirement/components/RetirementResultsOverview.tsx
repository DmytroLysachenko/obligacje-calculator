    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-0 rounded-lg bg-muted/30 md:grid-cols-3">
        <div className="border-b border-dashed border-border px-4 py-3 md:border-b-0 md:border-r">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">{labels.coverage}</p>
          <p className="mt-1 text-sm font-semibold text-foreground">
            {scenarioCoverage ?? formatHorizonMonths(inputsHorizonYears * 12, language)}
          </p>
        </div>
        <div className="border-b border-dashed border-border px-4 py-3 md:border-b-0 md:border-r">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">{labels.taxWrapper}</p>
          <p className="mt-1 text-sm font-semibold text-foreground">{taxStrategyLabel}</p>
        </div>
        <div className="px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">{labels.taxPaid}</p>
          <p className="mt-1 text-sm font-semibold text-foreground">{formatCurrency(totalTaxPaid)}</p>
        </div>
      </div>
